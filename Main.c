...
int main() {
	...
	do{
		...
		printf("\n-------------------------MENU-------------------------\n");
		printf("\n[+] Si quieres calcular la precisión del algoritmo pulsa 1. \n[+] Si quieres insertar un nuevo elemento a comprobar pulsa 2.\n[+] Si quieres aplicar wilson pulsa 3.\n\n\t[-] Escoja una opción: ");
		scanf(" %d", &x);
		printf("\n");
		...
		switch(x){
			case 1:
				...
				printf("Fin de la clasificación, para k = %d obtenemos:\n\n", K);
				...
				imprimirMatrizConfusion (fp, tp, tn, fn);
...
void imprimirMatrizConfusion(int fp, int tp, int tn, int fn) {
	printf("\t[-] Representación de la matriz de confusión: \n\n\t\t|\t\t\t|\n\t\t|\t%d\t%d\t|\n\t\t|\t%d\t%d\t|\n\t\t|\t\t\t|\n\n", fp, tp, tn, fn);
}
