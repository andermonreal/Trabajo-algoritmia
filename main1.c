...
// Meto todos los elementos a una lista para evitar compararme conmigo mismo y, comparo las distancias hacia mi con cada elemento
	while (c != NULL) {
		...
		struct ejemplo tmp; // Una auxiliar temporal donde voy poniendo los datos y metiendolos a la lista
		...
		tmp.approved = c->elem.approved; // FALTABA DE METER SI SE APROVABA O NO, SUPONGO QUE SERA IMPORTANTE
...
result obtenerClaseMasPrometedora(tipoLista lista, int K) {
	...
	int i = 0;
	celdaLista *c = lista.ini; // Para recorrer la lista
	while(c != NULL && i<K) { // recorremos los k vecinos mas proximos (PUEDE HABER UN FALLO EN LA CONADICION DEL BUCLE MIRARLO BIEN)
